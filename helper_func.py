...
async def check_banUser(filter, client, update):
    try:
        ...
        return await ocean.ban_user_exist(user_id)
...
async def check_admin(filter, client, update):
    try:
        user_id = update.from_user.id       
        return any([user_id == OWNER_ID, await ocean.admin_exist(user_id)])
    except Exception as e:
...
async def is_subscribed(filter, client, update):
    ...
    if any([user_id == OWNER_ID, await ocean.admin_exist(user_id)]):
        return True
    ...
    for id in Channel_ids:
        ...
        try:
            ...
            if REQFSUB and await privateChannel(client, id):
                if not await ocean.reqSent_user_exist(id, user_id):
                    return False
            else:
                return False
