...
rave = FILE_AUTO_DELETE
renzo = rave
file_auto_delete = humanize.naturaldelta(renzo)
...
async def start_command(client: Client, message: Message):
    ...
    if len(text)>7:
        ...
        for msg in messages:
            ...
            try:
                rave_msg = await msg.copy(chat_id=message.from_user.id, caption = caption, parse_mode = ParseMode.HTML, reply_markup = reply_markup, protect_content=PROTECT_CONTENT)
                await asyncio.sleep(3)
                rave_msg.append(rave_msg)
